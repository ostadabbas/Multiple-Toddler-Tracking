...
import numpy as np
from genetic_algo import GeneticOptimizer
...
frames_path = '/home/bishoymoussas/Workspace/MOT/R_MTT1_subsc/subscene_3'
yaml_dir = "/home/bishoymoussas/Workspace/MOT/YOLOv8-DeepSORT-Object-Tracking/ultralytics/yolo/v8/detect/deep_sort_pytorch/exp_configs/"

def predict(cfg, config_path):
    ...
    unique_filename = '_'.join(filename_parts) + '_preds.txt'
    ...
    init_tracker_with_config(config_path)
    ...
    # Evaluation
    gt_data_df = load_data('/home/bishoymoussas/Workspace/MOT/R_MTT1_subsc/subscene_3/gt.txt')
...
def main(cfg):
    ...
    config_template = {
        ...
        'NMS_MAX_OVERLAP': (0.2, 0.8),
        'MAX_IOU_DISTANCE': (0.2, 0.9),
        'MAX_AGE': (10, 200),
        'N_INIT': (2, 15),
        'NN_BUDGET': (20, 120)
    }
    ...
    # population = initialize_population(pop_size, yaml_dir)
    ...
    def fitness_function(config):
        config_path = convert_config_to_yaml_path(config)
        generate_yaml_from_config(config, yaml_dir)  # Create the YAML file, using the global yaml_dir
        metrics = predict(cfg, config_path)
        score = metrics['HOTA_Score'] + metrics['MOTA_Score'] + metrics['idf1']
        return -score


    # Run the Genetic Optimizer
    best_solution, best_fitness = optimizer.optimize(fitness_function)
